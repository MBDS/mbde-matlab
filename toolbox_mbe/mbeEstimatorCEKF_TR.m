classdef mbeEstimatorCEKF_TR < mbeEstimatorFilterBase
...
%             if (~isempty(obs))
                ...
                error = norm(g); 
                iter = 0; 
            %% Iterative loop for trapezoidal rule
                while error > me.tol_dyn && iter < me.iter_max
                    iter = iter+1;
                    %Rp = ace(q_next, R_next, l, x, 0); % Derivative of R_next with time 
                    %Rp = R;
                    R_q = me.bad_mech_phys_model.jacob_Rq(q_next,R_next);
                    %zpp = M_bar\Q_bar;
                    zpp = Xp_next(ind_idxs2);
                    for i = 1:length(me.iidxs)
                        Rp(:,i) = R_q(:,:,i)*qp_next;
                        M_Rq_R_zpp(:,i) = M*R_q(:,:,i)*R_next(:,i)*zpp(i);
                    end
                    F_21 = -(M_bar)\(R_next'*(2*M_Rq_R_zpp)); % was F_21 = -(M_bar)\(R'*(2*M*R_q*R*zpp));
                    [~, C] = me.bad_mech_phys_model.eval_KC(q_next,qp_next);
                    F_22 = -(M_bar)\(R_next'*(M*Rp + C*R_next));
                    F=[ zeros(me.lenZ)  ,   eye(me.lenZ);
                    F_21, F_22 ];
                    FP = F*P_next;            
            
                    Pp_next=FP+FP'+me.CovPlantNoise;
                    % Eval observation Jacobian wrt the indep. coordinates:
                    [dh_dz , dh_dzp] = me.bad_mech_phys_model.sensors_jacob_indep(me.q,me.qp,me.qpp);
                    H=[dh_dz, dh_dzp];   % Was: H = [dh_dq(:,me.iidxs) , dh_dqp(:,me.iidxs) ];
                    if (~isempty(obs))  % Only reduce uncertainty if we actually have an observation
                        Pp_next = Pp_next - ((P_next*H')/(me.CovMeasurementNoise))*H*P_next;
                    end            
                    P_next = me.P+0.5*(Pp+Pp_next)*me.dt; % Trapezoidal rule integration
                    % Kalman gain
                    K = (P_next*H')/(me.CovMeasurementNoise);
                    
                    % Update estimation error covariance matrix and Kalman
                    % gain
